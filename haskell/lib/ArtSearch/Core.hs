...
-- |
data ArtSearchConfig = ArtSearchConfig
...
-- | display the config
instance P.Show ArtSearchConfig where
  show c =
    T.printf
...
-- | constructs a default ArtSearchConfig
--
-- configHost:
--
...
-- @"art-search/0.1.0.0"@
--
newConfig :: IO ArtSearchConfig
newConfig = do
    logCxt <- initLogContext
    return $ ArtSearchConfig
        { configHost = "https://api.artsearch.io"
        , configUserAgent = "art-search/0.1.0.0"
...
addAuthMethod :: AuthMethod auth => ArtSearchConfig -> auth -> ArtSearchConfig
addAuthMethod config@ArtSearchConfig {configAuthMethods = as} a =
  config { configAuthMethods = AnyAuthMethod a : as}

-- | updates the config to use stdout logging
withStdoutLogging :: ArtSearchConfig -> IO ArtSearchConfig
...
withStderrLogging :: ArtSearchConfig -> IO ArtSearchConfig
...
withNoLogging :: ArtSearchConfig -> ArtSearchConfig
...
data ArtSearchRequest req contentType res accept = ArtSearchRequest
  { rMethod  :: NH.Method   -- ^ Method of ArtSearchRequest
  , rUrlPath :: [BCL.ByteString] -- ^ Endpoint of ArtSearchRequest
  , rParams   :: Params -- ^ params of ArtSearchRequest
  , rAuthTypes :: [P.TypeRep] -- ^ types of auth methods
  }
  deriving (P.Show)

-- | 'rMethod' Lens
rMethodL :: Lens_' (ArtSearchRequest req contentType res accept) NH.Method
rMethodL f ArtSearchRequest{..} = (\rMethod -> ArtSearchRequest { rMethod, ..} ) <$> f rMethod
{-# INLINE rMethodL #-}

-- | 'rUrlPath' Lens
rUrlPathL :: Lens_' (ArtSearchRequest req contentType res accept) [BCL.ByteString]
rUrlPathL f ArtSearchRequest{..} = (\rUrlPath -> ArtSearchRequest { rUrlPath, ..} ) <$> f rUrlPath
{-# INLINE rUrlPathL #-}

-- | 'rParams' Lens
rParamsL :: Lens_' (ArtSearchRequest req contentType res accept) Params
rParamsL f ArtSearchRequest{..} = (\rParams -> ArtSearchRequest { rParams, ..} ) <$> f rParams
{-# INLINE rParamsL #-}

-- | 'rParams' Lens
rAuthTypesL :: Lens_' (ArtSearchRequest req contentType res accept) [P.TypeRep]
rAuthTypesL f ArtSearchRequest{..} = (\rAuthTypes -> ArtSearchRequest { rAuthTypes, ..} ) <$> f rAuthTypes
{-# INLINE rAuthTypesL #-}
...
class HasBodyParam req param where
  setBodyParam :: forall contentType res accept. (Consumes req contentType, MimeRender contentType param) => ArtSearchRequest req contentType res accept -> param -> ArtSearchRequest req contentType res accept
  setBodyParam req xs =
...
class HasOptionalParam req param where
  ...
  applyOptionalParam :: ArtSearchRequest req contentType res accept -> param -> ArtSearchRequest req contentType res accept
  applyOptionalParam = (-&-)
  {-# INLINE applyOptionalParam #-}

  -- | infix operator \/ alias for 'addOptionalParam'
  (-&-) :: ArtSearchRequest req contentType res accept -> param -> ArtSearchRequest req contentType res accept
  (-&-) = applyOptionalParam
  {-# INLINE (-&-) #-}
...
-- ** ArtSearchRequest Utils

_mkRequest :: NH.Method -- ^ Method
          -> [BCL.ByteString] -- ^ Endpoint
          -> ArtSearchRequest req contentType res accept -- ^ req: Request Type, res: Response Type
_mkRequest m u = ArtSearchRequest m u _mkParams []

_mkParams :: Params
_mkParams = Params [] [] ParamBodyNone

setHeader ::
     ArtSearchRequest req contentType res accept
  -> [NH.Header]
  -> ArtSearchRequest req contentType res accept
...
removeHeader :: ArtSearchRequest req contentType res accept -> [NH.HeaderName] -> ArtSearchRequest req contentType res accept
...
_setContentTypeHeader :: forall req contentType res accept. MimeType contentType => ArtSearchRequest req contentType res accept -> ArtSearchRequest req contentType res accept
_setContentTypeHeader req =
    case mimeType (P.Proxy :: P.Proxy contentType) of
        Just m -> req `setHeader` [("content-type", BC.pack $ P.show m)]
        Nothing -> req `removeHeader` ["content-type"]

_setAcceptHeader :: forall req contentType res accept. MimeType accept => ArtSearchRequest req contentType res accept -> ArtSearchRequest req contentType res accept
...
addForm :: ArtSearchRequest req contentType res accept -> WH.Form -> ArtSearchRequest req contentType res accept
addForm req newform =
...
_addMultiFormPart :: ArtSearchRequest req contentType res accept -> NH.Part -> ArtSearchRequest req contentType res accept
...
_setBodyBS :: ArtSearchRequest req contentType res accept -> B.ByteString -> ArtSearchRequest req contentType res accept
_setBodyBS req body =
    req & L.set (rParamsL . paramsBodyL) (ParamBodyB body)

_setBodyLBS :: ArtSearchRequest req contentType res accept -> BL.ByteString -> ArtSearchRequest req contentType res accept
_setBodyLBS req body =
    req & L.set (rParamsL . paramsBodyL) (ParamBodyBL body)

_hasAuthType :: AuthMethod authMethod => ArtSearchRequest req contentType res accept -> P.Proxy authMethod -> ArtSearchRequest req contentType res accept
...
class P.Typeable a =>
  ...
  applyAuthMethod
    ...
    -> a
    -> ArtSearchRequest req contentType res accept
    -> IO (ArtSearchRequest req contentType res accept)
...
_applyAuthMethods
  ...
  -> ArtSearchConfig
  -> IO (ArtSearchRequest req contentType res accept)
_applyAuthMethods req config@(ArtSearchConfig {configAuthMethods = as}) =
  foldlM go req as
  where
